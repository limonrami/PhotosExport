...
func extractMetadata(asset: PHAsset, logger: LineLogger? = nil) async -> [String: Any] {
  ...
  // Fetch content editing input
  let contentInput: PHContentEditingInput? = await withCheckedContinuation { (cont: CheckedContinuation<PHContentEditingInput?, Never>) in
  ...
  guard let input = contentInput else { return metadata }

  metadata["contentUniformType"] = input.uniformTypeIdentifier
  metadata["fullSizeImageOrientation"] = input.fullSizeImageOrientation
  if let pairing = input.value(forKey: "livePhotoPairingIdentifier") as? String {
    metadata["livePhotoPairingIdentifier"] = pairing
  }
  if let adjustment = input.adjustmentData {
    metadata["adjustmentData"] = [
      "formatIdentifier": adjustment.formatIdentifier,
      "formatVersion": adjustment.formatVersion,
      "dataBase64": adjustment.data.base64EncodedString()
    ]
  }
      ...
      for idx in 0..<total {
        ...
        do {
          ...
          if settings.metadata {
            ...
            // Attach exported file info (type, filenames, sizes) to metadata payload.
